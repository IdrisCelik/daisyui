...
A [Claude Code frontend design plugin](/claude-code/ui-design-plugin/) gives your agent reusable UI guidance without pasting the same rules into every prompt. The daisyUI plugin packages the latest daisyUI skill for Claude Code.
...
For [frontend design](/claude-code/web-design-skill/), that power cuts both ways. Claude Code can build a screen quickly, but it can also create inconsistent UI quickly. Without shared design rules, it may write a dashboard that works yet feels like a collection of unrelated parts.
