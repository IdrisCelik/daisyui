...
If you prefer Cursor's plugin flow, install the [daisyUI plugin](/cursor/ui-design-plugin/). It provides the latest daisyUI skill through the plugin system.
...
The skill is compact static guidance. [Blueprint MCP](/cursor/ui-design-mcp-server/) can provide on-demand daisyUI context and component snippets while Cursor works. That can reduce repeated prompt text even further for larger UI tasks.
