...
That makes Cursor useful for [UI work](/cursor/how-to-design-ui/). You can ask it to create a dashboard, refactor a form, or add a responsive layout. But if your prompt only says "make it clean," Cursor has to invent the visual system.
...
This is not a Cursor-only problem. It happens when an [AI agent](/tailwind-css/agent/) receives a task but no design system.
...
The daisyUI plugin gives Cursor the latest [daisyUI skill](/cursor/free-design-skill/) through Cursor's plugin flow. Use it when you want the setup managed inside Cursor instead of installing the skill manually.
