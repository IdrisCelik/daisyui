...
To use less token with [Cursor](/cursor/use-less-tokens/), separate design context from the task prompt. Agent Mode should receive the screen goal, file target, and states. daisyUI context can come from a skill, plugin, or MCP server.
...
Plugin: open [Cursor settings](/cursor/ui-design-plugin/), go to Plugins, search for https://github.com/saadeghi/daisyui, choose daisyui, and click Add to Cursor.
