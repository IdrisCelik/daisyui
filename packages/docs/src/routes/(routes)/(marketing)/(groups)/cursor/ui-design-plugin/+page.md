...
A UI design plugin for Cursor keeps daisyUI guidance inside the editor workflow. Cursor plugins can bundle rules, skills, commands, [MCP servers](/cursor/ui-design-mcp-server/), agents, and hooks. For daisyUI, the plugin gives Agent Mode access to design skill instructions.

## Why use the plugin path

A pasted prompt is temporary. A [plugin](/cursor/use-less-tokens/) stays available through Cursor settings and can be managed with other project or user extensions. That is useful when you generate UI often and want one repeatable setup.
