...
To use less token with Codex, stop writing a design manual inside every UI prompt. Put [reusable daisyUI guidance](/codex/ui-design-plugin/) in a skill or MCP server, then keep prompts focused on the feature.

## Why this works

Codex skills use progressive disclosure. Codex sees skill metadata first and reads full instructions only when needed. [Blueprint MCP](/codex/ui-design-mcp-server/) gives another option: fetch daisyUI details during the task.
...
After installing the skill or MCP server, remove repeated low-level design rules from your [Codex prompts](/codex/ui-design-skill/). Do not restate every button, radius, color, and theme instruction. Put those in daisyUI context.
