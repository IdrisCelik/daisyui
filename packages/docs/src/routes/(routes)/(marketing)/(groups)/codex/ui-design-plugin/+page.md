...
A UI design plugin for Codex is the installable way to add daisyUI design guidance to Codex. Codex plugins can bundle skills, app integrations, and [MCP servers](/codex/ui-design-mcp-server/), but the key value here is the daisyUI skill packaged for reuse.

## Plugin versus project dependency

The [Codex plugin](/codex/use-less-tokens/) does not replace installing daisyUI in your app. The app still needs daisyUI CSS if you want the classes to render. The plugin helps Codex choose the right classes while it writes UI code.
